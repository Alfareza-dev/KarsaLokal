...
import { Plus, Trash2, Save, Sparkles, ArrowLeft, X } from "lucide-react";
...
export default function AdminEditProductPage({
  ...
  const [formData, setFormData] = useState({
    ...
    currentPrice: "",
    description: "",
    status: "ready" as "ready" | "limited" | "habis",
    isTerlaris: false,
    isBaru: false,
    isActive: true,
    weight: "",
  ...
  // Resolve dynamic route param (Next.js 16 async params)
  useEffect(() => {
    params.then(({ id }) => setProductId(Number(id)));
  }, [params]);

  // Fetch categories and product data together
  useEffect(() => {
    if (!productId) return;

    const fetchData = async () => {
      setIsLoadingData(true);
      const [{ data: catData }, { data: configData }, { data: product, error }] = await Promise.all([
      ...
      if (configData) setPaymentMode(configData.payment_mode || "manual");

      if (error) {
        console.error("[EditProduct] Fetch error:", error);
        toast.error(`Produk tidak ditemukan: ${error.message}`);
      ...
      if (product) {
        setFormData({
          ...
          normalPrice: product.normal_price?.toString() || "",
          currentPrice: product.current_price?.toString() || "",
          description: product.description || "",
          status: (product.status as "ready" | "limited" | "habis") || "ready",
          isTerlaris: product.is_best_seller ?? false,
          isBaru: product.is_new ?? false,
  ...
  const handleInputChange = (
    ...
    if (type === "checkbox") {
      ...
      // Slug field: strip anything that isn't lowercase letters, digits, or hyphens
      if (name === "slug") {
  ...
  const handleQuickAddCategory = async () => {
    ...
    const { data, error } = await supabase
      .from('categories')
      .insert([{ id, name: newCat.name, emoji: newCat.emoji, order_index: nextOrder, is_active: true }])
      .select();
  ...
  if (isLoadingData) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="w-10 h-10 rounded-full border-4 border-zinc-200 border-t-pink-400 animate-spin" />
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 pb-20">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">
              Perbarui informasi produk di katalog Anda
            </p>
          </div>
        </div>
        ...
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="hidden sm:flex items-center gap-2 bg-zinc-950 hover:bg-zinc-800 text-white px-6 py-2.5 rounded-xl font-bold shadow-none transition-all disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isSaving ? (
              ...
              {paymentMode === "manual" && (
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 bg-white border border-zinc-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-zinc-300 transition-all text-zinc-950 appearance-none"
                  >
                    <option value="ready"> Ready</option>
                    <option value="limited">️ Terbatas</option>
      ...
      <div className="fixed bottom-0 left-0 w-full p-4 bg-zinc-50 border-t border-zinc-200 sm:hidden z-40">
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="w-full flex justify-center items-center gap-2 bg-zinc-950 hover:bg-zinc-800 text-white px-6 py-3.5 rounded-xl font-bold shadow-none disabled:opacity-60 disabled:cursor-not-allowed transition-all"
        >
          {isSaving ? (
            <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
          ) : (
      ...
      <AnimatePresence>
        {showCategoryModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <button onClick={() => setShowCategoryModal(false)} className="absolute top-4 right-4 p-2 bg-white text-zinc-500 rounded-full hover:bg-gray-200 transition-colors"><X size={16} /></button>
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <input type="text" value={newCat.name} onChange={(e) => setNewCat({...newCat, name: e.target.value})} placeholder="Contoh: Kuliner Lokal" className="w-full px-4 py-2.5 bg-white border border-zinc-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-zinc-300 transition-all text-zinc-950" />
                </div>
                <div className="grid grid-cols-1 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-zinc-950 mb-1.5">Icon Emoji</label>
                    <input type="text" value={newCat.emoji} onChange={(e) => setNewCat({...newCat, emoji: e.target.value})} placeholder="" className="w-full px-4 py-2.5 bg-white border border-zinc-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-zinc-300 transition-all text-zinc-950 text-center" />
                  </div>
                </div>
                <button type="button" onClick={handleQuickAddCategory} disabled={isSavingCat} className="w-full bg-gray-900 hover:bg-gray-800 text-white py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-60">
                  {isSavingCat ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : null}
