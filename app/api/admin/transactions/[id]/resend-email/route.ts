...
export async function POST(
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabaseAdmin
      .from('orders')
      .select('id, user_id, status, reference_id, product_name, payment_method, amount, shipping_method')
      .eq('id', orderId)
      .single();
    ...
    if (order.status !== 'settled') {
      return NextResponse.json({ error: 'Order is not settled yet' }, { status: 400 });
    }
    ...
    if (!customerEmail) {
      console.error("[Resend] Fetch user error:", userError);
      return NextResponse.json({ error: 'Customer email not found', details: userError }, { status: 400 });
    }

    // 3. Ambil konfigurasi toko untuk email template & info
...
`;
    ...
    try {
      await sendEmail({
        ...
        to: customerEmail,
        subject: `[Kirim Ulang] Pesanan Diproses - ${order.reference_id || order.id}`,
        html: emailHtml,
      });
    } catch (err) {
      console.error('Failed to re-send fulfillment email', err);
      return NextResponse.json({ error: 'Gagal mengirim email' }, { status: 500 });
    }

    return NextResponse.json({ success: true, message: 'Email has been successfully resent.' }, { status: 200 });

  } catch (error: any) {
    console.error('Manual resend email failed:', error);
